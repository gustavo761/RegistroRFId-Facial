...
pathBackup = 'C:/RegistroRfidFacial/Reportes'
...
def reporteIndividual(datos):
    ...
    pathGuardarReporte = f'{pathBackup}/{now.day}-{now.month}-{now.year} {now.hour}-{now.minute}-{now.second} Individual.xlsx'
...
def reporteGeneral(datos):
    ...
    pathGuardarReporte = f'{pathBackup}/{now.day}-{now.month}-{now.year} {now.hour}-{now.minute}-{now.second} General.xlsx'
    wb = load_workbook(pathPlantilla)
    ws = wb['Hoja1']
